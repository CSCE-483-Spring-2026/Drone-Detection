...
def load_hf_non_drone_dataset():
    ...
    none_drone_ds = none_drone_ds.shuffle(seed=42).select(range(900))  # select 1200 non-drone samples, which will be further split into train and valid sets

    return none_drone_ds

def load_local_drone_audio_dataset(local_dir="/home/luke_gut/Drone-Detection/droneDataV2"):
    ...
    for class_folder, label in label_map.items():
        ...
        print(f"{class_folder}: found {class_count} wav files -> label {label}")
